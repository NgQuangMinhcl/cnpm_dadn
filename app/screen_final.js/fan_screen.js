...
const Fan_screen = () => {
  ...
  const postToAdafruitIO = async () => {
    if (feedData.last_value != null) {
      try {
        ...
        let newData;
        if (feedData.last_value == "1") {
          newData = {
            value: "2",
          };
        } else if (feedData.last_value == "2") {
          newData = {
            value: "1",
          };
        }
  ...
  const fetchAdafruitData = () => {
    const feedUrl = "https://io.adafruit.com/api/v2/chuloi/feeds/fan";
    axios
      .get(feedUrl)
      .then((response) => {
    ...
    <View>
      ...
      >
        ...
        >
          ...
          >
            <Text>Loại thiết bị : quạt</Text>
            <MaterialCommunityIcons name="fan" size={40} color="black" />
            <Text>
              Trạng thái hiện tại :
              {feedData
                ? feedData.last_value === "1"
                  ? "ON"
                  : feedData.last_value === "2"
                  ? "OFF"
                  : "Chưa có dữ liệu"
                : "Không có dữ liệu"}
            </Text>
