...
const chartConfig = {
  ...
  const Temp_screen = () => {
   ...
   const fetchTempData = () => {
      const feedUrl = 'https://io.adafruit.com/api/v2/minhnguyenquang/feeds/welcome-feed/data';
      axios.get(feedUrl)
        .then((response) => {
    ...
    const run = () =>{
      if (feed != null) {
        ...
        if(d>30){
           setNote("Nhiệt độ hiện tại rất nóng, bạn nên uống nhiều nước hơn và hạn chế ra ngoài");
        }
        else if(d>25 && d <=30){
           setNote("Nhiệt độ hiện tại có dấu hiệu nóng lên, bạn nên cẩn thận");
        }
        else if(d >20 && d <=25){
          setNote("Nhiệt độ hiện tại rất tốt, thích hợp cho các hoạt động ngoài trời");
        }
        else if(d<20 || d == 20){
   ...
   return (    
    ...
    <Text style={{fontSize:20,marginLeft:120,fontWeight:'bold'}}>Nhiệt độ phòng</Text>
    </View>
    ...
    <View style={{
          ...
          <Text style ={{marginTop:10,fontSize:15}}>Nhiệt độ hiện nay so với 100 °C</Text>
          <ProgressChart
            data={chartdata}
            width={300}
            height={220}
    ...
    <View style={{
          ...
          <Text style ={{marginTop:10,fontSize:15}}>Chú ý: Nhiệt độ hiện tại là {feed ? feed[0].value : 'Chưa có dữ liệu'} °C</Text>
