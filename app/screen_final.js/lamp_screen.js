...
const Lamp_screen = () => {
  ...
  const postToAdafruitIO = async () => {
    if (feedData.last_value != null) {
      try {
        ...
        let newData;
        if (feedData.last_value == "3") {
          newData = {
            value: "4",
          };
        } else if (feedData.last_value == "4") {
          newData = {
            value: "3",
          };
        }
  ...
  const fetchAdafruitData = () => {
    const feedUrl = "https://io.adafruit.com/api/v2/chuloi/feeds/led";
    axios
      .get(feedUrl)
      .then((response) => {
        ...
        >
          ...
          >
            ...
            <Text>
              Trạng thái hiện tại :
              {feedData
                ? feedData.last_value === "3"
                  ? "ON"
                  : feedData.last_value === "4"
                  ? "OFF"
                  : "Chưa có dữ liệu"
                : "Không có dữ liệu"}
            </Text>
